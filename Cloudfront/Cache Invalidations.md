[[CloudFront]] distributions have a cache which has an TTL (Time to Live) to avoid requesting contents from it's origins all the time, especially when you're using it along a [[S3]] bucket.
